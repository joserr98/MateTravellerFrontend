      ...
      {trips.length !== 0 ? (
        <div className="tripsContainer">
          <div className="trips">
            {trips.map((trip) => (
              <div className="trip">
                <div className="tripsCity">{trip.city}</div>
                <div className="tripsDates">
                  <div className="startDate">{trip.start_date}</div>
                  <div className="endDate">{trip.end_date}</div>
                </div>
                <div className="tripsDescription">{trip.description}</div>
                <div><Button onClick={() => {trip.id}}>Join Trip</Button>{trip.id}</div>
              </div>
            ))}
          </div>
          <div className="pagination">
            <ButtonToolbar aria-label="Toolbar with button groups">
              <ButtonGroup className="me-2">
                {page != 1 ? (
                  <Button onClick={() => setPageFunction(1)}>1</Button>
                ) : (
                  <></>
                )}
                {page != 1 ? (
                  <Button onClick={() => setPageFunction(page - 1)}>
                    Previous
                  </Button>
                ) : (
                  <></>
                )}
                <Button onClick={() => setPageFunction(page + 1)}>Next</Button>
                {page != pages ? (
                  <Button onClick={() => setPageFunction(pages)}>
                    {pages}
                  </Button>
                ) : (
                  <></>
